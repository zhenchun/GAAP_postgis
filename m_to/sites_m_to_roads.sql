		...
		from '|| recpt ||' as r
	        ...
	        from
		     ...
		     on nn.nn_all = t.gid
                ...
                sql := 'drop table if exists sites_m_to_roads;
                        create table sites_m_to_roads as
                        select '|| recpt||'.id
                        from '|| recpt||'';
			execute sql;
           foreach p in array type	
           loop
                     ...
                     sql :=' alter table sites_m_to_roads add m_to_sh'|| p ||' double precision;
                     update sites_m_to_roads set m_to_sh'|| p ||'='|| p ||'.distance from '|| p ||' where sites_m_to_roads.id= '|| p ||'.id';   
                execute sql;
                end loop;
