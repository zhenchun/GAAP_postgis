		...
		with intsct as (
			       ...
			       where st_intersects(c.geom, b.b'|| i ||')
                         ...
                         select SFS.id, coalesce(SFS.Surface, 0) as lu_surface_s' || i || ',
							   (coalesce(CRP.Cropland, 0)+coalesce(FRT.Forest, 0)+coalesce(GRD.Grassland, 0)+coalesce(SRD.Shrubland, 0))/coalesce(SFS.Surface, 0) as lu_green_s' || i || ',
							   (coalesce(WET.Wetland, 0)+coalesce(WAT.Water, 0))/coalesce(SFS.Surface, 0) as lu_blue_s' || i || ',
							   (coalesce(IPV.Impersfc, 0)+coalesce(BAR.Bareland, 0))/coalesce(SFS.Surface, 0) as lu_grey_s' || i || '
                          from
