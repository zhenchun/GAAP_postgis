...
declare
	...
	radii text[] = array['3000','1500','1000', '750','500','400','300','150', '100', '50'];
	i text;
	sql text;
begin
    ...
    select SFS.id, coalesce(SFS.Surface, 0) as surface,
		coalesce(CRP.Cropland, 0) as cropland,
    coalesce(FRT.Forest, 0) as forest,
    coalesce(GRD.Grassland, 0) as grassland,
    coalesce(SRD.Shrubland, 0) as shrubland,
    coalesce(WET.Wetland, 0) as wetland,
    coalesce(WAT.Water, 0) as water,
    coalesce(IPV.Impervious, 0) as impervious,
    coalesce(BAR.Bareland, 0) as bareland
    from
    ...
    on SFS.id = WAT.id
    left join
      (select b.id, coalesce(intsct.area, 0) as Impervious
      from buffers as b left join intsct
      on b.id = intsct.id
      where intsct.dn = ''80''
      group by b.id
