#########################https://gis.stackexchange.com/questions/14456/finding-the-closest-geometry-in-postgis
...
declare 
	...
	roads text := 'shcd_crs4499'; --constant_nor_lur_bng_dists, all_vehsum, allmv
	sql text;
begin
	--find nearest neighbours
	sql := '
	drop table if exists m_to_shcd;
	create table m_to_shcd as 
	with nn as (
		select distinct r.id, r.geom, 
		nnid(r.geom, 1000, 2, 100, '''|| roads ||''', ''gid'', ''geom'') as nn_all
		from '|| recpt ||' as r
	)
	select DMR.id, DMR.distnear, (1 / DMR.distnear) as distinvnear1
	from
		(select nn.id, st_distance(nn.geom, t.geom) as distnear
